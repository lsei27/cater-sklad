...
import { Download, Image as ImageIcon, Plus, Search, Upload } from "lucide-react";
...
export default function AdminItemsPage() {
  ...
  const exportCsv = async () => {
    ...
    setExporting(true);
    try {
      const { stocks } = await api<{ stocks: Record<string, Record<string, number>> }>("/inventory/warehouse-stocks");
      ...
      const sortedWarehouses = [...warehouses].sort((a: any, b: any) => a.name.localeCompare(b.name, "cs"));
      // Ledger entries created before a warehouse was assigned have no warehouseId; surface
      // that leftover quantity as its own column so the warehouse_* columns always sum to total_quantity.
      const unassignedQty = (item: any) => {
      ...
      const warehouseColumns = [
        ...
        ...(hasUnassigned ? [{ id: null as string | null, header: "warehouse_unassigned" }] : [])
      ];
      ...
      const headers = [
        ...
        "volume",
        "plate_diameter",
        ...warehouseColumns.map((w) => w.header),
      ...
      const rows = items.map((item) => {
        ...
        return [
          ...
          item.volume ?? "",
          item.plateDiameter ?? "",
          ...warehouseColumns.map((w) => (w.id === null ? unassignedQty(item) : (stocks[item.id]?.[w.id] ?? 0))),
      ...
      toast.error(e?.error?.message ?? "Export se nezdařil.");
    } finally {
      setExporting(false);
    }
  };
